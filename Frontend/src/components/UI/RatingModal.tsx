...
import { Rating, Store } from '@/types';   // ✅ import Store
import { ratingsAPI } from '@/services/api';

interface RatingModalProps {
  store: Store; // ✅ Added store prop
  rating?: Rating;
  open: boolean;
    ...
    try {
      if (rating) {
        ...
        await ratingsAPI.create({
          ...
          rating: ratingValue,
          comment,
          userName: 'Current User', // TODO: Replace with actual user.name from AuthContext
          value: 0
        });
      }
      await onSave();
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>
            {rating ? 'Edit Rating' : `Rate ${store.name}`} {/* ✅ can use store name */}
          </DialogTitle>
        </DialogHeader>
